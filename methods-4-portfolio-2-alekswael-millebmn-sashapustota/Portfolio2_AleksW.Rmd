...
```{r}
adjustmentSets(dag, exposure = "M", outcome = "B")
```
...
Fr = ceiling(rlnorm(n=300, location, shape))

B = 6 + 2*log(Fr)
B = as.numeric(B)

test = tibble(x = B, y = Fr)

ggplot(data = test) +
  geom_point(mapping = aes(y, x)) +
  geom_smooth(mapping = aes(y, x), method = "lm")
...
# At 50 people, when there's live music, you will drink 2.5 beers more.
b3 = rnorm(300, 0.05, 0.01)

# How many beers do you consume?
#B = rpois(300, 6 + b1*M + 2*log(Fr) + b3*D)
hist(B)

df = tibble(M = M, Fr = Fr, D = D, B = B)

m1 = lm(B ~ M + D + Fr, data = df)
summary(m1)
